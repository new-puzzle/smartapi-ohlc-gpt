...
def get_token_from_symbol(symbol: str, exchange: str = "NSE"):
    ...
    raise HTTPException(status_code=404, detail=f"Symbol '{symbol}' not found on exchange '{exchange}'. Please provide a valid NSE stock ticker.")

@app.get("/api/get-ohlc")
def get_ohlc_data(stock_symbol: str, exchange: str = "NSE", days: int = 30):
    if not all([API_KEY, API_SECRET, ANGEL_USERNAME, ANGEL_PASSWORD, ANGEL_TOTP]):
    ...
    try:
        ...
        session_data = smart_api.generateSession(ANGEL_USERNAME, ANGEL_PASSWORD, ANGEL_TOTP)
